...
//-------------------------------------------------------------------
/*/{Protheus.doc} CBL00103
Notas Fiscais de Emissão Própria
...
oBrowse:SetDescription( 'TIB XML - PRÉ NOTA EMISSÃO PRÓPRIA' )
...
oBrowse:SetFilterDefault("SZ1->Z1_CGCEMI == SM0->M0_CGC .AND. !(SZ1->Z1_TPXML == '3')")
...
ADD OPTION aRotRel  Title 'Pend. Escrit.'   Action 'U_PLFISR03' 		OPERATION 2 ACCESS 0

ADD OPTION aRotina Title 'Visualizar'  			Action 'U_VERNOTA' OPERATION 2 ACCESS 0
//ADD OPTION aRotina Title 'Liberar'				Action 'U_XMLLIB2' 			OPERATION 3 ACCESS 0
ADD OPTION aRotina Title 'Classificar' 			Action 'U_ClassNo3'			OPERATION 4 ACCESS 0
ADD OPTION aRotina Title 'Alterar'        	    Action 'U_AltXML'			OPERATION 4 ACCESS 0
...
ADD OPTION aRotina Title 'Vis. Pré-Nota'		Action 'VIEWDEF.CBL00103'   OPERATION 2 ACCESS 0
...
Local aCposSZ1 := {}
Local nC       := 0
Local aCpoAltSZ2 := {"Z2_DESPESA"} //Campos que poderão ser alterados
...
/*
If (ALTERA .AND. SZ1->Z1_TPXML == '1') //Caso esteja em Alteração e Nota Fiscal de Entrada
	aAux := oStruSZ2:GetFields()
	//Definindo Todos os Campos com WHEN .F.
	For nI := 1 To Len(aAux)
		oStruSZ2:SetProperty( aAux[nI, 03] , MODEL_FIELD_WHEN, {||.F.})
	Next nI
	//Definindo os campos que poderão ser alterados somente em Alteração
	For nI := 1 To Len(aCpoAltSZ2)
		oStruSZ2:SetProperty( aCpoAltSZ2[nI] , MODEL_FIELD_WHEN, {||.T.})
	Next nI
EndIf
*/
...
//oModel:SetRelation( 'SZ2DETAIL', { { 'Z2_FILIAL', 'xFilial( "SZ2" )' }, { 'Z2_DOC', 'Z1_DOC' } 		}, SZ2->( IndexKey( 1 ) ) )
//oModel:SetRelation( 'SZ2DETAIL', { { 'Z2_FILIAL', 'xFilial( "SZ2" )' }, { 'Z2_DOC', 'Z1_DOC' } 		}, SZ2->( IndexKey( 18 ) ) )
...
Local oModel   := FWLoadModel( 'CBL00103' )
Local oView
...
Local aInfo		:= {}
Local aPosGet	:= {}
Local aTpNf     := {"N=Normal", "B=Beneficiamento", "D=Devolução", "C=Compl. Preço", "I=NF Compl. ICMS", "P=NF Compl. IPI"}
...
cCpoSZ1 += " Z1_DOC|Z1_SERIE|Z1_TIPO|Z1_EMISSAO|Z1_FORNECE|Z1_LOJA|Z1_ESPECIE|Z1_EST|Z1_COND"
...
//Verificar se possui o CFOP para incluir ComboBox no campo
If (u_VldCpoZ1(SZ1->Z1_DOC, SZ1->Z1_SERIE, SZ1->Z1_FORNECE, SZ1->Z1_LOJA , SZ1->Z1_TIPO, 'Z1_TIPO'))
	oStruSZ1:SetProperty('Z1_TIPO', MVC_VIEW_COMBOBOX, aTpNf)
EndIf
...
If cId == "S"
	Do Case
		...
		Case SZ1->Z1_STATUS == '5' // NF Importada (Pendente de Reclassificação)
			cLeg := "BR_MARROM"
		Case SZ1->Z1_STATUS == '6' // NF Denegada
			//cLeg := "BR_CINZA"
			cLeg := "BR_LARANJA"
		Case SZ1->Z1_STATUS == '7' // Bloqueio de movimento
			cLeg := "BR_AZUL"
	EndCase
Else
	Do Case
...
If cId == "S"
	...
	oLegenda:Add("","BR_LARANJA"	,"NF Denegada"	)
	oLegenda:Add("","BR_AZUL"		,"Bloqueio de Movimento"	)
Else
...
Static Function ChangeLine(oModel, oView, cViewID)
...
MaFisAlt("IT_PRODUTO"	, oMdlSZ2:GetValue("Z2_COD")	,1)
MaFisAlt("IT_QUANT"		, oMdlSZ2:GetValue("Z2_QUANT")	,1)
MaFisAlt("IT_PRCUNI"	, oMdlSZ2:GetValue("Z2_VUNIT")	,1)
MaFisAlt("IT_VALMERC"	, oMdlSZ2:GetValue("Z2_TOTAL")	,1)
MaFisAlt("IT_ALIQIPI"	, oMdlSZ2:GetValue("Z2_IPI")	,1)
MaFisAlt("IT_TES"		, oMdlSZ2:GetValue("Z2_TES") 	,1)
